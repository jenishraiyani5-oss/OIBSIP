...
The commands below are provided for reference/documentation purposes only,
reflecting the exact steps performed in Section 2 against the author's own
local DVWA lab instance. `<your_cookie_here>` must be replaced with a live
session cookie captured from an authenticated browser session — the values
below are not usable as-is and are not intended to be run against any
...
sqlmap -u "http://127.0.0.1:42001/vulnerabilities/sqli/" \
  ...
  --cookie="PHPSESSID=<your_cookie_here>; security=medium" \
  --batch
```
